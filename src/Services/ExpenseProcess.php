...
class ExpenseProcess implements ExpenseContract
{
    ...
    private function processTransaction(string $type, string $urlPath): ExpenseProcess
    {
        ...
        $this->expenseRequestBody['amount'] = $this->credentials['amount'] ??
            $this->walletResponse['data']['transaction']['amount'];
        $this->expenseRequestBody['description'] = $this->credentials['description'] ??
            $this->walletResponse['data']['transaction']['description'] ?? $type;
        $this->expenseRequestBody['reference'] =  $this->walletResponse['data']['transaction']['reference'] ??
            $this->reference;

        $this->expenseResponse = sendRequestTo(
            $transactionUrl,
    ...
    private function accessUserBvnAndEmail(): void
    {
        ...
        $this->expenseRequestBody['bvn'] = $profileInstance->query()
                ->selectRaw((string) $bvnColumn)
                ->whereUserId((int) $this->credentials['user_id'])
                ->first()->{$bvnColumn} ?? null;

        $this->expenseRequestBody['email'] = $userInstance->query()
                ->selectRaw((string) $emailColumn)
                ->whereId((int) $this->credentials['user_id'])
...
//        }
     ...
     * @throws ExpenseException
     */
    public function getBillTransaction(string $reference): array
    {
